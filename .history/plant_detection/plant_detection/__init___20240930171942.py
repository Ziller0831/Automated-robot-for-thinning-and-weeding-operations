...
class PlantDetector:
    ...
    def adjust_exposure(self, exposure_value):
        if self.color_sensor.supports(rs.option.exposure):
            self.color_sensor.set_option(rs.option.enable_auto_exposure, 0)  # Disable auto exposure
    ...
    def adjust_laser_power(self, laser_power_value):
        if self.depth_sensor.supports(rs.option.laser_power):
            self.depth_sensor.set_option(rs.option.laser_power, laser_power_value)
    ...
    def detect_objects(self):
        ...
        # 保存捕获的帧作为备份
        image_path = os.path.join(self.unlabeled_directory, 'realsense_capture1.jpg')
        ...
        centroid_x_list, centroid_y_list, class_list = self._load_coordinates_from_csv(csv_filename)
        filtered_points, removed_points = self._filter_points(centroid_x_list, centroid_y_list)
        self._display_points(image, filtered_points, removed_points)
        
        # 将移除的点保存到CSV文件中
        removed_csv_filename = os.path.join(self.output_directory, "removed_points1.csv")
    ...
    def _save_to_csv(self, image_path, object_list):
        csv_filename = os.path.join(self.output_directory, os.path.splitext(os.path.basename(image_path))[0] + '.csv')
    ...
    def _display_points(self, image, filtered_points, removed_points):
        for (x, y) in filtered_points:
            cv2.circle(image, (int(x), int(y)), 5, (0, 255, 0), -1)  # Green for filtered points
        for (x, y) in removed_points:
            cv2.circle(image, (int(x), int(y)), 5, (0, 0, 255), -1)  # Red for removed points
        
        # Save the image with points to the output directory
        output_image_path = os.path.join(self.output_directory, 'points_display1.jpg')
